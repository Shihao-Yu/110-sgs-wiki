...
4. 触发列表页和详情页的 `revalidatePath`（沿用项目 `revalidate-map` 模式）。
5. 返回最新的 `{ counts, total, topTier }`。
...
**新增**：
- `packages/data/src/types/rating.ts` — `RatingTier`, `RATING_TIERS`, `GeneralRating`, `topTier()`
- `packages/data/src/types/index.ts` — 导出
- `packages/web/src/lib/ratings-store.ts` — KV adapter（也可放进 `entity-store.ts`，看实现复杂度）
...
**修改**：
- `packages/web/src/lib/entity-store.ts` — 加 `getRatings()` / 触发重算
